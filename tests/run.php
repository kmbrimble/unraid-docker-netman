...
require_once __DIR__ . '/../plugin/include/multinet.php';
...
$parsed = multinet_parse_extra($extraParams, 'br0', $rows);
...
check('extra: rows round-trip', multinet_rows_equal($parsed['rows'], $rows));

// re-serialize what we parsed and parse again -> identical
$run2 = multinet_build_network_run('br0', $parsed['rows'], $parsed['primary_mac'], false);
$extraParams2 = multinet_serialize_extra($parsed['remaining'], $run2);
checkEquals('extra: idempotent serialize', $extraParams, $extraParams2);
$parsed2 = multinet_parse_extra($extraParams2, 'br0', $rows);
check('extra: idempotent parse', multinet_rows_equal($parsed2['rows'], $rows));

// ---- ExtraParams path: primary-with-MAC (fact 3) -----------------------

$macRun = multinet_build_network_run('proxynet', [], '02:11:22:33:44:55', true);
checkEquals('mac: primary carries mac-address', '--network name=proxynet,mac-address=02:11:22:33:44:55', $macRun);
$macParsed = multinet_parse_extra($macRun, 'proxynet', []);
...
$noMacRun = multinet_build_network_run('br0', [], '02:11:22:33:44:55', false);
...
$parsedForeign = multinet_parse_extra($foreign, 'br0', []); // no state.json entry recorded
...
$known = multinet_build_network_run('br0', $knownRows, null, false);
$parsedKnown = multinet_parse_extra($known, 'br0', $knownRows);
...
$parsedDrifted = multinet_parse_extra($known, 'br0', $driftedExpected);
...
$parsedPost = multinet_parse_post($postArgs, 'terrible-butler', $postRows);
...
check('post: rows round-trip', multinet_rows_equal($parsedPost['rows'], $postRows));
...
$combined = multinet_serialize_post($userPostArgs, $chain);
...
$parsedCombined = multinet_parse_post($combined, 'terrible-butler', $postRows);
checkEquals('post: user args preserved verbatim', $userPostArgs, $parsedCombined['remaining']);
check('post: rows still round-trip with user args present', multinet_rows_equal($parsedCombined['rows'], $postRows));

// idempotent re-serialize
$chain2 = multinet_build_connect_chain('terrible-butler', $parsedCombined['rows']);
$combined2 = multinet_serialize_post($parsedCombined['remaining'], $chain2);
...
$parsedForeignPost = multinet_parse_post($foreignPost, 'terrible-butler', []);
check('post manually-managed: no state entry -> refuse', $parsedForeignPost['manually_managed']);

// ---- IP validation -------------------------------------------------------

check('ip in subnet', multinet_ip_in_subnet('172.18.0.5', '172.18.0.0/16'));
check('ip not in subnet', !multinet_ip_in_subnet('10.0.0.5', '172.18.0.0/16'));
check('ip in subnet /24', multinet_ip_in_subnet('172.18.0.254', '172.18.0.0/24'));
check('ip outside /24', !multinet_ip_in_subnet('172.18.1.1', '172.18.0.0/24'));

// ---- XML template write preserves everything else -----------------------

$tmpXml = sys_get_temp_dir() . '/multinet-test-' . getmypid() . '.xml';
...
$ok = multinet_write_template_field($tmpXml, 'PostArgs', '--foo bar && docker network connect --ip 172.18.0.3 proxynet terrible-butler');
...
$ok2 = multinet_write_template_field($tmpXml, 'ExtraParams', '');
...
checkEquals('path: bridge -> post', 'post', multinet_choose_path('bridge'));
checkEquals('path: host -> post', 'post', multinet_choose_path('host'));
checkEquals('path: none -> post', 'post', multinet_choose_path('none'));
checkEquals('path: container:x -> post', 'post', multinet_choose_path('container:other'));
checkEquals('path: br0 -> extra', 'extra', multinet_choose_path('br0'));
checkEquals('path: proxynet -> extra', 'extra', multinet_choose_path('proxynet'));
